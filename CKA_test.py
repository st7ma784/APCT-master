...
Dataset=torch.utils.data.TensorDataset(torch.rand(16000,3,227,227))
...
for x1 in tqdm(dataloader):
    ...
    cka.res_matrix = torch.div(cka.m1_matrix, torch.sqrt(torch.abs(torch.mul(cka.hsic_matrix,cka.m2_matrix))))
