...
class LightningCLIPModule(LightningModule):
    ...
    def forward(self, im, captions1, captions2, captions3, captions4, captions5):
        image_features=self.encode_image(im)@torch.inverse(self.text_projection)
    ...
    def validation_step(self,batch,*args):
        ...
        self.log("delta solve - proj",torch.sum(torch.linalg.solve(captions,image_features.T.float(),left=False)-self.text_projection), prog_bar=True,enable_graph=False)
        self.log("delta solve - projinv",torch.sum(torch.linalg.solve(captions,image_features.T.float(),left=False)-torch.inverse(self.text_projection)), prog_bar=True,enable_graph=False)
