...
class parser(HyperOptArgumentParser):
    def __init__(self,*args,strategy="random_search",**kwargs):
        ...
        self.opt_list("--learning_rate", default=0.0001, type=float, options=[1e-5, 2e-4,1e-4,5e-5], tunable=True)
        self.opt_list("--batch_size", default=9, type=int, options=[6,8,10], tunable=True)
