...
class LightningCLIPModule(LightningModule):
    ...
    def training_step(self, batch, batch_idx,optimizer_idx=0):
        ...
        lossim = self.loss(logs*torch.einsum('az,bz,cz,dz,ez->abcdez',cache1,cache2,cache3,cache4,cache5).matmul(cacheim.T),labels)
        ...
        loss1 = self.loss(logs*torch.einsum('az,bz,cz,dz,ez->abcdez',cache2,cache3,cache4,cache5,cacheim).matmul(caption_features1.T),labels)
        ...
        loss2 = self.loss(logs*torch.einsum('az,bz,cz,dz,ez->abcdez',cache3,cache4,cache5,cacheim,cache1).matmul(caption_features2.T),labels)
        ...
        loss3 = self.loss(logs*torch.einsum('az,bz,cz,dz,ez->abcdez',cache4,cache5,cacheim,cache1,cache2).matmul(caption_features3.T),labels)
        ...
        loss4 = self.loss(logs*torch.einsum('az,bz,cz,dz,ez->abcdez',cache5,cacheim,cache1,cache2,cache3).matmul(caption_features4.T),labels)
        ...
        loss5 = self.loss(logs*torch.einsum('az,bz,cz,dz,ez->abcdez',cache1,cache2,cache3,cache4,cacheim).matmul(caption_features5.T), labels)
...
def wandbtrain(config=None,dir="/Data",devices="auto",accelerator="auto",Dataset=None):
    with wandb.init(project="6DIMCachecontSweep",entity="st7ma784",config=config) as run:

        logtool= pytorch_lightning.loggers.WandbLogger( project="6DIMCachecontSweep",entity="st7ma784",experiment=run, save_dir=dir)
